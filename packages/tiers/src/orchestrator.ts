...
export async function scrape(
  ...
  if (!req.skipHttp && !skipTier1ForProxy && maxTier >= 1) {
    const t1 = await runTier1(req.url, sanitizedHeaders, req.method, req.body, req.cookies, tier1Proxy)
    emit(t1)
  ...
  try {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      t3 = await tier3Runner(req.url, handle, remaining3, proxy3, sanitizedHeaders, req.method, req.body, req.cookies)
